...
# Due Date: 6/9/23
# Description: Hashmap implementation using Open Addressing with Quadratic Probing
...
class HashMap:
    ...
    def put(self, key: str, value: object) -> None:
        ...
        Else, it is added.
        """
        #checking if table needs resizing
        if self.table_load() >= 0.5:
            self.resize_table(self._capacity * 2)

        initial = self._hash_function(key) % self._capacity
        index = initial
        #if index is empty place HashEntry object there
        if self._buckets[initial] is None or self._buckets[initial].is_tombstone is True:
            self._buckets.set_at_index(initial, HashEntry(key, value))
            self._size += 1
        #else if key already exists, update value
        elif self._buckets[initial].key == key:
    ...
    def get(self, key: str) -> object:
        """
        Returns value associated with given key. Returns None if key is not in hash table.
        """
        initial = self._hash_function(key) % self._capacity
        index = initial
        j = 1
    ...
    def contains_key(self, key: str) -> bool:
        ...
        #use quadratic probing until whole array is probed or key is found
        initial = self._hash_function(key) % self._capacity
        index = initial
        j = 1
        while self._buckets[index] is not None:
    ...
    def remove(self, key: str) -> None:
        ...
        #using quadratic probing and setting tombstone value to true if key is found
        while self._buckets[index] is not None:
            if self._buckets[index].key == key and self._buckets[index].is_tombstone is False:
    ...
    def get_keys_and_values(self) -> DynamicArray:
        """
        Returns a dynamic array where each index contains a tuple of a key/value pair from hash map.
        """
        array = DynamicArray()
        for i in range(self._buckets.length()):
            if self._buckets[i] is not None and self._buckets[i].is_tombstone is False:
                array.append((self._buckets[i].key, self._buckets[i].value))
